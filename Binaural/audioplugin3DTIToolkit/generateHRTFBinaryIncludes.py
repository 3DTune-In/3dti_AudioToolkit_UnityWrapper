...
input_files = [
    ...
    os.path.join(*'HighPerformance/ILD/HRTF_ILD_44100.3dti-ild'.split('/')),
    os.path.join(*'HighPerformance/ILD/HRTF_ILD_48000.3dti-ild'.split('/')),
    os.path.join(*'HighPerformance/ILD/HRTF_ILD_96000.3dti-ild'.split('/')),
]
output_directory = os.path.join(script_dir, *'src_autogenerated'.split('/'))
...
namespace Spatializer3DTI
{
    ...
    extern const std::map<std::string, std::basic_string<uint8_t>> listenerBinaries;
#endif
}
...
#ifndef SPATIALIZER3DTI_EXCLUDE_DEFAULT_LISTENER_BINARIES
    const std::map<std::string, std::basic_string<uint8_t>> listenerBinaries {{
''')
    outer_indent = ' '*4
